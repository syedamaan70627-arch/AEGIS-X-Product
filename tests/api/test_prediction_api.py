"""
Tests for AEGIS-X API Failure Prediction Endpoints.
"""
...
@pytest.fixture
def setup_prediction_resources():
    """Sets up a registered model and evaluation dataset with prediction risk features."""
    ...
    res_m = client.post(
        "/api/v1/models",
        data={"model_name": "Prediction Test Model", "task_type": "binary_classification"},
...
def test_prediction_unfitted_artifact_returns_not_available(setup_prediction_resources):
    """Test that querying prediction without a fitted artifact returns NOT_AVAILABLE (HTTP 200)."""
    res = setup_prediction_resources
...
def test_prediction_fitted_artifact_returns_available(setup_prediction_resources):
    """Test that fitted predictor artifact executes prediction successfully."""
    res = setup_prediction_resources
    ...
    df_train = pd.DataFrame({
        ...
        "Failure_Onset_Next": [0, 1, 0, 1, 0],
    })
    df_val = df_train.copy()
    predictor.fit(df_train, df_val, target_column="Failure_Onset_Next")

    artifact_dir = settings.ARTIFACTS_DIR / res["model_id"]
    artifact_dir.mkdir(parents=True, exist_ok=True)
    joblib.dump(predictor, artifact_dir / "prediction_model.joblib")
