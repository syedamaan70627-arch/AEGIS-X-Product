          ...
          >
            {latestAnalysis ? (
              <div className="space-y-4">
                ...
                <div className="p-3 bg-slate-950/60 border border-slate-800/80 rounded-xl text-[11px] font-mono text-slate-400 flex flex-col sm:flex-row sm:items-center justify-between gap-2">
                  <span>Latest Analysis ID: <strong className="text-slate-200">{latestAnalysis.analysis_id}</strong></span>
                ...
                <table className="w-full text-left text-xs border-collapse">
                  <thead className="bg-slate-950/80 text-slate-400 uppercase font-mono tracking-wider text-[11px] border-b border-slate-800">
                    <tr>
                      ...
                      <th className="p-3.5">Fusion Method</th>
                      <th className="p-3.5">Fused Risk</th>
                      <th className="p-3.5">True Target Labels</th>
                  ...
                  <tbody className="divide-y divide-slate-800/60">
                    {recentAnalyses.map((a) => (
                      <tr key={a.analysis_id} className="hover:bg-slate-800/40 transition-colors">
                        ...
                        <td className="p-3.5 font-medium text-slate-200 capitalize">{a.fusion_method}</td>
                        ...
                        <td className="p-3.5">
                          {a.has_labels ? (
                            <span className="text-emerald-300 font-mono text-[11px] font-semibold px-2 py-0.5 rounded bg-emerald-950/60 border border-emerald-800/60">
                              Present
                            </span>
                          ) : (
                            <span className="text-slate-400 font-mono text-[11px]">Absent (Label-Free)</span>
                          )}
                        </td>
                        <td className="p-3.5 text-slate-400 font-mono text-[11px]">{new Date(a.created_at).toLocaleString()}</td>
